...
def classify_question(question: str) -> List[str]:
    ...
    """
    prompt = "You are a Project Manager. Do not use any tools. Classify the following question and select the top 1 most relevant Subject Matter Expert. "
    prompt += f"Question: {question}."
    prompt += 'Respond with the top 1 SME in the following JSON template. Do NOT print anything else! {"smes": ["sme", "sme", "sme"]}: '
...
def consult_smes(question: str, selected_smes: List[str]) -> Dict[str, str]:
    ...
    try:
        ...
        print(f"An error occurred while running the agent for {sme}. Error: {e}")
        responses[sme] = "An error occurred while running the agent."
    return responses
...
def resolve_best_answer(question, answers):
    ...
    """
    agent = initialize_agent(tools, llm, agent=AgentType.STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION, verbose=True)
    prompt = "You are a resolver tasked with finding which of the answer options the Subject Matter Experts have provided is the best answer. Let's work this out in a step by step way to be sure we have the right answer. Do not use tools.\n\n"
